<x-guest-layout>
    <div class="mb-8">
        <h3 class="font-bold text-2xl text-gray-900 mb-2">Buat Akun Baru 🚀</h3>
        <p class="text-gray-500">Lengkapi data di bawah untuk mulai mencari kos idamanmu.</p>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-5">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="text" name="name" value="{{ old('name') }}"
                    ...
                    class="block w-full pl-10 pr-3 py-3 border border-gray-200 rounded-xl focus:ring-primary focus:border-primary sm:text-sm bg-gray-50 focus:bg-white transition-colors @error('name') border-red-500 focus:ring-red-500 focus:border-red-500 @enderror" />
            </div>
            @error('name')
        ...
        <div class="mb-2">
            ...
            <div class="relative">
                ...
                <input id="password_confirmation" type="password" name="password_confirmation"
                    ...
                    class="block w-full pl-10 pr-10 py-3 border border-gray-200 rounded-xl focus:ring-primary focus:border-primary sm:text-sm bg-gray-50 focus:bg-white transition-colors" />
                <button type="button" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-gray-600 focus:outline-none"
        ...
        <div class="mb-2 mt-4">
            <label class="block text-sm font-bold text-gray-700 mb-2">Verifikasi: Berapa hasil dari? <span class="text-red-500">*</span></label>
            <div class="flex items-center gap-4 bg-primary/5 p-4 rounded-xl border border-primary/10">
                <span class="font-extrabold text-primary text-xl tracking-wider">
                    @php
                        $num1 = rand(1, 10);
                        $num2 = rand(1, 10);
                ...
                <input type="number" name="captcha" required placeholder="Jawaban Anda" 
                    class="block w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-primary focus:border-primary sm:text-sm bg-white transition-colors shadow-sm text-center font-bold text-gray-900 text-lg">
            </div>
            @error('captcha')
                <p class="mt-2 text-sm text-red-600 font-medium"><i class="fa fa-exclamation-circle mr-1"></i>{{ $message }}</p>
            @enderror
        ...
        <button type="submit" class="w-full flex justify-center py-3.5 px-4 border border-transparent rounded-xl shadow-md text-sm font-bold text-white bg-primary hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-all hover:shadow-lg hover:-translate-y-0.5">
            Buat Akun Sekarang <i class="fa fa-user-plus ml-2 mt-0.5"></i>
        </button>
        ...
        <p class="text-center text-gray-500 text-sm mt-8 font-medium">
            Sudah punya akun?
            <a href="{{ route('login') }}" class="font-bold text-primary hover:text-primary/80 transition-colors">Masuk di sini</a>
        </p>
    </form>
